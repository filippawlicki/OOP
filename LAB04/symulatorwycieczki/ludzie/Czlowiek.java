...
public abstract class Czlowiek implements Uczestnik {
    ...
    protected void regeneruj(double czas) {
        ...
        System.out.printf("Poziom zmęczenia u %s %s zmalał o %.2f i wynosi %.2f. ",imie, nazwisko, zregenerowano, poziomZmeczenia);
    }
    ...
    protected void aktualizujZmeczenie(double czas)
    {
        ...
        System.out.printf("Poziom zmęczenia u %s %s wzrósł o %.2f i wynosi %.2f. ",imie, nazwisko, dodatkoweZmeczenie, poziomZmeczenia);
    ...
    public void opiszWycieczke(Wycieczka wycieczka) {
        ...
        for(int i=0; i<wycieczka.getLiczbaElementowWycieczki(); i++) {
            ...
            if(elementWycieczki instanceof Wedrowka) {
                ...
                mow(lp+". Atrakcja: "+atrakcja.getNazwa() + " spędzimy tam " + atrakcja.getCzasZwiedzaniaAtrakcji() + " h");
            }
            else {
